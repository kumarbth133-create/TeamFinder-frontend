        ...
        <div className="grid lg:grid-cols-2 gap-5">
          ...
          <div className="card">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-sm font-semibold text-gray-100">Recent Students</h2>
              <Link to="/admin/students" className="text-xs text-green-400 hover:text-green-300 transition">View all</Link>
            </div>
            <div className="space-y-3">
              {stats.recentStudents.map((s) => (
                <div key={s._id} className="flex items-center justify-between">
                  ...
                  <p className="text-xs text-gray-600">{new Date(s.createdAt).toLocaleDateString()}</p>
                </div>
              ))}
              {stats.recentStudents.length === 0 && (
                <p className="text-xs text-gray-600 text-center py-4">No students yet</p>
              )}
            </div>
          </div>
          ...
          <div className="card">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-sm font-semibold text-gray-100">Recent Projects</h2>
              <Link to="/admin/projects" className="text-xs text-green-400 hover:text-green-300 transition">View all</Link>
            </div>
            <div className="space-y-3">
              {stats.recentProjects.map((p) => (
                <div key={p._id} className="flex items-center justify-between">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full ${
                    p.status === "open" ? "bg-primary-800/50 text-green-400" : "bg-gray-800 text-gray-500"
                  }`}>{p.status}</span>
                </div>
              ))}
